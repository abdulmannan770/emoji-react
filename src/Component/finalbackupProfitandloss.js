...
class ProfitandLoss extends Component {
    ...
    var  fincode = this.props.fincode;
     await axios.get(`http://172.104.5.85:3000/demoProfitandLoss/`+fincode+'/'+rtype+'/'+cstype, {
          ...
          if(data.Table1.length>0){
            ...
            var allnextkeys=allkeys.slice(1);
            allnextkeys=allnextkeys.reverse();
            //console.log(allnextkeys.length);
            this.setState({
              pldata:data.Table1,      
              pldataobject:data.Table1[0],
              allnextkeys1:allnextkeys,
  ...
  render(){
    ...
    if(allnextkeys1.length>0){
        var allnextkeys=allnextkeys1.slice(1);
                                  ...
                                  value.Particulars==="Interest" || value.Particulars==="Depreciation"?
                                   <td key={key.toString()}> 
                                      {Object.keys(allnextkeys1).map(key2 => 
                                          key===allnextkeys1[key2] ? 
                                            (parseFloat(value[key])<parseFloat(value[allnextkeys1[key2-1]])) ? 
                                                <span key={key2} className='greenDark' >
                                                    <span style={this.state.displayPLvalue}>{value[key]}</span>
                                                    <span style={this.state.displayVariance} >{value[key]-value[allnextkeys1[key2-1]]!==0 && value[allnextkeys1[key2-1]]!==0 ? parseFloat(((value[key]-value[allnextkeys1[key2-1]])*100)/value[allnextkeys1[key2-1]]).toFixed(2)+'%' : '' }</span>
                                                </span> 
                                            : (parseFloat(value[key])>parseFloat(value[allnextkeys1[key2-1]])) ? 
                                      ...
                                      {/*<span style={this.state.displayVariance} >{Object.keys(allnextkeys1).map(key2 => key===allnextkeys1[key2] && key2!=0 ? parseFloat(((value[key]-value[allnextkeys1[key2-1]])*100)/value[key]).toFixed(2)+'%' : '')}</span>*/}
                                    </td>
                                  :<td key={key.toString()}> 
                                      {Object.keys(allnextkeys1).map(key2 => 
                                          key===allnextkeys1[key2] ? 
                                            ...
                                            : key2!=0 ? <span key={key2} className='greenDark' >
                                                          <span style={this.state.displayPLvalue}>{value[key]}</span>
