...
import Peercompare from './Component/Peercompare';
import Bulkblockdeals from './Component/Bulkblockdeals';
import Corporateactions from './Component/Corporateactions';
import Balancesheet from './Component/Balancesheet';
import Cashflow from './Component/Cashflow';
import Financialratios from './Component/Financialratios';
import Announcement from './Component/Announcement';
import Profitandloss from './Component/Profitandloss';
import Shareholding from './Component/Shareholding';
...
class App extends Component {
  constructor(props){
    ...
    this.state = {
       ...
       weekness:[],
       basicDetails: {},
       scoreparameter:{efficiency:'',growth:'',liquidity:'',trust:''},
       techScore:0,
       note:'',
    }
  }
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (currentParams.fincode && currentParams.fincode !== prevParams.fincode) {
      console.log(currentParams.fincode);
    ...
    componentDidMount() {
      ...
      this.getNewStrengthWeekness(fincode);
      this.getGraphFunda(fincode);
      this.getGraphprice(fincode);
      this.getTechnicalScore(fincode);
    }
    basicDetailsHanlder = (basicDataList) => {
      this.setState({ basicDetails: basicDataList  })
    }
    /************Total Score************************/
    getTotalScore = async (fincode) => {
          ...
          if(data.subfundascore.length>0){
            this.setState({
              scoreparameter:{efficiency:parseFloat(data.subfundascore[0].efficiency).toFixed(0),
    ...
    getTechnicalScore = async (fincode) => {
      await axios.get(`http://172.104.5.85:3000/getTechnicalScore/`+fincode, {
          ...
          var data = responseText.data;
          this.setState({
              techScore:parseFloat(data.data).toFixed(0),
          })
      }).catch(function (error) {});
    }
    /************Strength Weekness************************/
 		getNewStrengthWeekness = async (fincode) => {
      await axios.get(`http://172.104.5.85/accorddemo/index.php/welcome/newstrengthweekness/`+fincode, {
	          headers: new Headers({"Content-Type": "application/json"}),
	    }).then(responseText => {
	        var data = responseText.data;
    ...
    /************Fundamental score Graph************************/
    getGraphFunda = async (fincode) => {
      await axios.get(`http://172.104.5.85:3000/graphfunda/`+fincode, {
          headers: new Headers({"Content-Type": "application/json"}),
      }).then(responseText => {
          var data = responseText.data;
          if(data.length>0){
    ...
    getGraphprice = async (fincode) => {
        await axios.get(`http://172.104.5.85:3000/graphprice/`+fincode, {
            headers: new Headers({"Content-Type": "application/json"}),
        }).then(responseText => {
            var data = responseText.data;
            if(data.Table.length>0){
              ...
              for(var i=0;i<data.Table.length;i++){
                  allprice.push(data.Table[i].price);
              }
              this.setState({
                allprice:allprice,
              })
  ...
  render() {
  	const {error,fscore,strengthdata,weekness,alllabels,fundavalues,allprice,techScore,note} = this.state; 
    ...
    {/********Header *************/}
      ...
      <article className="aticle">
            ...
            <div className="mainContainer">
              <div className="mainScore clearfix">
                <div className="fundaScore">
                  ...
                  <img src={fscore<25 ? "/img/liquidity.png" : fscore>=25 && fscore<50 ? "/img/growth.png" : fscore>=50 && fscore<75 ? "/img/trustss.png" : fscore>=75 ? "/img/trust.png"  : '' } alt="fscore" height={30}/>
                  {/*<img src="/img/img_fundamentel.png" alt="Fundamentel" />*/}
                  <span>{fscore}</span>
                </div>
                <div className="fundaScore techni">
                  <h2>TECHNICAL SCORE</h2>
                  <img src={techScore<25 ? "/img/liquidity.png" : techScore>=25 && techScore<50 ? "/img/growth.png" : techScore>=50 && techScore<75 ? "/img/trustss.png" : techScore>=75 ? "/img/trust.png"  : "/img/liquidity.png" } alt="techScore" height={30}/>
                  <span>{techScore}</span>
                </div>
              </div>
            </div>
        ...
        <div id="contents">
          ...
          {/********Basic Detail*************/}
            <Basicdetails {...this.state} basicDataList={this.basicDetailsHanlder} />
              ...
              <div className="row mt10">
                ...
                <div className="col-md-8">
                  <div className="strengthWeek">
                    ...
                    <span className="swnote">{note ? "Note: "+note:null}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="invest">
              <div className="mainContainer">
                <h2 className="mainHead">INVESTOR PRESENTATION <span>Reliance Industries Ltd.</span></h2>
                <div className="row mt10">
